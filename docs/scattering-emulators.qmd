...
|  Name | Functional for $K$ | Strong Form | Trial Basis | Test Basis | $\lambda$ |
|---|---|---|---|---|---|
|  Kohn ($\lambda$) |  $\trial K_E + \braket{\trial\psi | H - E | \trial\psi}$ | $H\ket{\psi} = E\ket{\psi}$ | $\ket{\psi_i}$ | $\bra{\psi_i}$  |  Yes |
| Kohn ($\slashed{\lambda}$)  | $\begin{aligned} \braket{\trial\chi | H - E | \trial\chi} + \braket{\phi | V | \trial\chi} \\ +\braket{\phi | H - E | \phi} + \braket{\trial\chi | V | \phi} \end{aligned}$  |  $[E-H]\ket{\chi} = V\ket{\phi}$ |  $\bra{\chi_i}$ | $\ket{\chi_i}$  | No  |
...
## Kohn Emulators with Lagrange Multipliers {#sec-kohn-lagrange}
...
However, other flavors exist for other matrices such at $T^{\pm}$ and $S$ (see Section @sec-kohn-general).
...
We start with a rescaled version of the KVP discussed in Furnstahl _et al._ [@Furnstahl:2020abp]:
$$
...
where $\phi(r) = j_\ell(qr)$ is the (regular) free-space wave function, and $j_\ell(qr)$ and $n_\ell(qr)$ are spherical Bessel and Neumann functions, respectively.
...
\begin{align} %\label{#eq-delta_u_tilde}
    ...
    %& \quad + \braket{\psi_j | [H - E] | \psi_i} \notag\\
    & = \braket{\psi_i | [V(\params) - V_j] | \psi_j}  + (i\leftrightarrow j) .\notag\\
    %&  \quad+ \braket{\psi_j | [V(\params) - V_i] | \psi_i} .
\end{align}
$$ {#eq-delta_u_tilde}
...
## Kohn Emulators without Lagrange Multipliers {#sec-kohn-no-lagrange}
...
However, one can instead build in the normalization constraint from the very start, thus removing the need to constrain our basis during emulation.
...
Melendez _et al._ [@Melendez:2021lyq] studied several applications of the emulator equation @eq-nvp-emulator to short-range potentials with and without the Coulomb interaction and partial-wave coupling.
They demonstrated that the NVP emulator has remarkable extrapolation capabilities and can quickly reproduce high-fidelity calculations of neutron-proton cross sections based on modern chiral interactions with negligible error.
...
For more details on solving the radial Schrödinger equation and matching the solutions to the asymptotic boundary condition @eq-psi-normalization, see, _e.g._, Reference [@thompson_nunes_2009].
As we can see from Figure @fig-wave-functions-origin, each training wave function and emulated wave function has matching boundary conditions at the origin, and the discrepancy from the true wave function is less than $10^{-3}$.
...
\psi_{\ell,E}^{\text{(free)}}(r) =  \bar{\psi}_{\ell,E}^{\text{(0,free)}}(r) + L_{\ell,E} \, \bar{\psi}_{\ell,E}^{\text{(1,free)}}(r) \, ,
\end{align}
...
\begin{pmatrix}
    \bar{\psi}_{\ell,E}^{\text{(0,free)}}(r)\\
    \bar{\psi}_{\ell,E}^{\text{(1,free)}}(r)
\end{pmatrix} =
\mathcal{N}^{-1}
\begin{pmatrix}
    u_{00} & u_{01}\\
...
Here, $L_{\ell, E}$ is a generic scattering matrix that is determined by the boundary condition, as parametrized by the nonsingular matrix $u$.

We now define $\genkvp$ as a general functional for the generic $L$-matrix in Equation @eq-free-sol-coordinate [@PhysRevA.40.6879;@Drischler:2021qoy;@KVP_vs_NVP:2022],
$$
\begin{equation} %\label{#eq-kvp-general}
    \mathcal{L}[\psi] = L + \frac{\mathcal{N}}{\mathrm{det} \, \umatrix} \braket{\psi_{\umatrix} | [H - E] | \psi_{\umatrix}}.
\end{equation}
$${#eq-kvp-general}
...
However, Drischler _et al._ [@Drischler:2021qoy] demonstrated that a KVP-based emulator that simultaneously emulates an array of KVPs with different boundary conditions can be used to systematically detect and remove these anomalies.
The anomalies are detected by assessing the consistency of the emulated results and, when detected, mitigated by adapting the emulator's training points to shift the anomaly's position in the parameter space.
The basic idea is general and can be applied to other emulators, including the NVP-based emulator discussed in Section @sec-newton, as long as multiple scattering boundary conditions can be emulated independently and efficiently.


### Generalization to coupled systems {#sec-kohn-coupled}

Following Garcia _et al._ [@KVP_vs_NVP:2022], let us now extend the generalized KVP in Section @sec-kohn-general to coupled systems, which could be coupled partial-wave or reaction channels.
...
This independence can be understood as follows.
...
Here, we follow Zhang \& Furnstahl [@Zhang:2021jmi] who developed KVP-based emulators for three-body systems.
...
```{python}
#| label: fig-yamaguchi_phase_shifts
#| fig-cap: "Phase shifts (top panel) and absolute residuals (bottom panel) for the Yamaguchi potential @eq-yamaguchi. The solid black lines represent the high-fidelity solution and the dots represent the different emulators results: NVP (blue) and KVP emulator (orange). The training set is given by the five gray lines."
p_valid = np.array([-30, 0, 20])
...
```{python}
svp.coefficients(p_valid).sum(axis=-1)
```


Figure @fig-yamaguchi_phase_shifts shows the phase shifts and the absolute residuals for the Yamaguchi potential @eq-yamaguchi.
The top panel depicts the high-fidelity solution (solid black curve) and the emulator results (dots) with blue representing the NVP emulator and orange the KVP emulator.
In gray we show the basis states used to train the emulator in the offline stage.
The bottom panel shows the absolute residuals for both emulators with the same color scheme as the top panel.
We can see that the NVP emulator accurately reproduces the high-fidelity solution, while the KVP emulator is not as accurate and diverges from the simulator $q \approx 1.5 \, \text{fm}^{-1}$.
...
```{python}
#| label: fig-yamaguchi_wave_functions
#| fig-cap: "Wave functions (top panel) and absolute residuals (bottom panel) for the Yamaguchi potential @eq-yamaguchi. The top panel legend description is similar to Figure @fig-yamaguchi_phase_shifts, but for three different values of $q$ in units of fm$^{-1}$. The bottom panel shows the relative residuals of the three values previously mentioned."
...
Figure @fig-yamaguchi_wave_functions shows the high-fidelity (solid black line), emulated (dots), and basis (solid gray line) wave functions for three values of $q$ with their absolute residuals.
...
```{python}
#| label: fig-minnesota_wave_functions
#| fig-cap: "Similar to @fig-yamaguchi_wave_functions but for the Minnesota potential."
...
```{python}
#| label: fig-minnesota_phase_shift
