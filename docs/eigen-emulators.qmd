...
In the following, we will discuss how the Ritz variational principle and the Galerkin method can be used to construct rapid and reliable^[A \emph{reliable} emulator may not necessarily be required to be highly accurate, _e.g._, if the other uncertainties of the theoretical calculation dominate the overall uncertainty budget.] emulators that facilitate these calculations. % and how they relate to more general projection approaches such as the Galerkin method.
...
\end{equation}
$$ {#eq-ritz_functional}
%combined with the _effective_ trial wave functions $\ket{\trial\psi}$ we will discuss below.
...
% _i.e._, $\ket{\testfunc_i} = \ket{\bar\testfunc_i} = \ket{\psi_i}$.
...
%Although the idiosyncrasies of the solver used for this task are irrelevant for our discussion, for illustration, we express here the eigenvalue problem @eq-generic_eigenvalue_problem in a truncated model basis in which only $N_h$ of the lowest-lying states are considered and perform a direct diagonalization of the $\nsimulator \times \nsimulator$ Hamiltonian matrix, as shown in Figure @fig-illustration_fom_rom.
