...
Throughout, we note the relationship between the variational principles from which emulators have been constructed in recent works.
...
$$
This formulation requires a matrix solve of one fewer dimension than @eq-coeff_solution and @eq-kohn-solution-no-lagrange tends to be a better conditioned system to solve as well.
...
$$
\begin{aligned}
    \mathcal{L}[\psi] = L - \frac{2\mu}{q} \braket{\psi | [H - E] | \psi}
\end{aligned}
$$ {#eq-kvp-general}
...
Suppose that the incoming and outgoing channels are labeled by $s'$ and $s$, respectively.
...
Here, each partial wave is completely independent of one another and thus it would not make sense to mix the VPs or the $\coeff$ across choices of $\ell$.
...
But for the off-diagonal channels, the test functions have a different outgoing channel ($s$) than the trial functions ($s'$), which is instead a Petrov-Galerkin approach.
...
The SVP is known to have better numerical properties comparied to the KVP, including the removal of so-called spurious singularities [CITE].
Furthermore, @Takatsuka1981SchwingerKohnRelationship showed that one can write down a variational principle of increasing order, of which the KVP is the leading order and the SVP is second order.
