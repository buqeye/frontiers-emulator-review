...
In this Section, we describe various reduced-basis emulators one could construct for quantum scattering systems.
...
For simplicity, we restrict our discussion to two-body scattering for Hermitian Hamiltonians. (See, however, Section @sec-higher-body-continuum for an extension to higher-body systems.
...
where $\phi(r) = j_\ell(qr)$ is the (regular) free-space wave function, and $j_\ell(qr)$ and $n_\ell(qr)$ are spherical Bessel and Neumann functions, respectively.^[We focus here on examples with real-valued potentials and without long-range Coulomb interactions. Cases with complex-valued potentials and/or the Coulomb interaction may be analyzed in similar ways; relevant discussions specific to Kohn emulators can be found in References [@Furnstahl:2020abp;@Drischler:2021qoy].]
Note that we define the on-shell $K_E$ matrix as
$$
\begin{equation}  % \label{eq:def_K_E}
...
$$
\begin{align} %\label{#eq-int-by-parts-wronskian}
    0
    & = \braket{\testfunc | H - E | \psi} \notag\\
    & = \braket{\testfunc | \overleftarrow{H} - E | \psi} - \left.\frac{W(r\testfunc, r\psi; r)}{2\mu} \right|_{r=0}^{\infty},
\end{align}
...
In conclusion, using the Galerkin projection of the homogeneous Schrödinger equation with trial and test bases of $\psi_i$, we were able is equivalent to obtain the same coefficients as the KVP as in Equation @eq-kohn-psi-coeff-solution, which yield the same on-shell $K$ matrix when used in Equation @eq-kohn-psi-K-emulator.
...
\begin{align} %\label{#eq-kohn-no-lagrange-solution}
   \Omega \coeffsopt = \vec{\omega},
\end{align}
$${#eq-kohn-no-lagrange-solution}
where
$$
\begin{align} % \label
    \Omega_{ij} & = \braket{\chi_i | [E - H] | \chi_j}, \\
    \omega_i & = \braket{\chi_i | V | \phi},
\end{align}
$${#eq-kohn-unconstrained-reduced-matrices}
which is the set of equations used to obtain $\coeffsopt$.
The matrix elements $\Omega_{ij}$ can be evaluated with the help of
$$
\begin{align}
...
\begin{equation} %\label{#eq-schwinger-vp}
    \mathcal{K}[\trial\psi] = \braket{\trial\psi | V | \phi} + \braket{\phi | V | \trial\psi} - \braket{\trial\psi | V - V G_0 V | \trial\psi},s
\end{equation}
$${#eq-schwinger-vp}
where $G_0$ is the Green's operator.
...
In fact, the NVP does not even require $\ket{\phi}$ and $\ket{\phi'}$ to be free-space states; one can impose stationarity of Equation @eq-nvp between any two states due to its operator form.
...
To compute phase shifts, we must emulate $K$ at the on-shell energy $E=q^2/2\mu$ and thus, $k = k' = q$ for $\ket{\phi}=\ket{k\ell m}$ and $\bra{\phi'}=\bra{k'\ell'm'}$.
...
By imposing the stationary condition $\partial \mathcal{K} /\partial \coeffs = 0$, one then finds
...
We now repeat the derivation for the NVP emulator, but instead from the perspective of a Galerkin projection.
Here, we will focus on the case where $\bra{\phi'} = \bra{\phi}$.
...
Although Equation @eq-LS is written in terms of abstract operators, it can be turned into a vector equation in a specific representation after right-multiplying by $\ket{\phi}$.
To derive the weak form we left-multiply by $G_0$ and a test function $\bra{\testfunc}$:
$$
\begin{align}
    \braket{\testfunc | G_0 K -  G_0 V G_0 K | \phi} & = \braket{\testfunc | G_0 V | \phi} .
\end{align}
$$
The trial function in this case is $K\ket{\phi}$, which can be expanded in a discrete (snapshot) basis using @eq-TritzTrial.
We further employ the Galerkin prescription, where the test basis is equivalent to the trial basis, making $\bra{\testfunc_i} = \bra{\phi} K_i$.
With these assumptions, the reduced weak form becomes
$$
\begin{align}
    M\coeffsopt = \vec{m},
\end{align}
$$
with $M$ and $\vec{m}$ defined in Equation @eq-nvp-reduced-mat-and-vec, again with $\bra{\phi'} = \bra{\phi}$.
Thus, we find the same $\coeffsopt$ using either the NVP or the Galerkin projection.

Given the optimal coefficients $\coeffsopt$, the emulator can be derived by substituting $\trial K$ into the right-hand side of Equation @eq-LS
$$
\begin{align}
    \braket{\phi | K |\phi} & \approx \braket{\phi | V |\phi} + \braket{\phi | V G_0 \trial K |\phi} \\
    & = \braket{\phi | V |\phi} + \frac{1}{2} \vec{m}^\trans M^{-1} \vec{m},
\end{align}
$$
which is equivalent to Equation @eq-nvp-emulator under the assumption that $\bra{\phi'} = \bra{\phi}$.
...
In the KVP, for example, the second constraint was obtained via the normalization of $r\psi(r)$ as $r\to\infty$, which, in the constrained KVP, led to a normalization constraint for the coefficients $\coeff_i$.
...
#| fig-cap: "Results for the scattering emulator with origin boundary conditions in arbitrary units. Six basis functions are shown as gray lines, and the exact wave function as a black line. Each of the basis functions and the emulated wave function satisfy the constraints at the origin."
bc_idx = 0
...
The parameters to be varied are $\params = \{\theta_1, \theta_2\}$.
The six training and one validation parameters are selected randomly from a uniform distribution in the range of $[-5, 5]$.
To obtain the snapshots, the partial-wave decomposed radial Schrödinger equation for $\ell = 0$ can be expressed as the system of coupled first-order differential equations,
$$
\begin{equation}
\begin{pmatrix}
    y'_0(r)\\
...
\begin{pmatrix}
    (r\psi)'(r) \\
    2\mu \left[V(r; \theta_i)  - E \right] (r\psi)(r)
\end{pmatrix},
\end{equation}
$$
...
For simplicity, let us consider short-range potentials $V(\params)$ that have been partial-wave decomposed into an uncoupled channel with angular momentum $\ell$.
The general asymptotic form of the (coordinate-space)} radial wave functions will be linear combinations of free-space solutions^[We follow the conventions for scattering matrices in References [@taylor2006scattering,@Morrison:2007]. Thus, the $K$ matrix in this section is dimensionless and defined without the negative sign in Eq. @eq-def_K_E.]
$$
\begin{align} %\label{#eq-free-sol-coordinate}
\psi_{\ell,E}(r) \xrightarrow[r\rightarrow\infty]{} \bar{\phi}_{\ell,E}^{\text{(0)}}(r) + L_{\ell,E} \, \bar{\phi}_{\ell,E}^{\text{(1)}}(r) \, ,
\end{align}
...
\begin{equation} %\label{#eq-kvp-general}
    \mathcal{L}[\trial\psi] = L_{\ell, E} + \frac{2 \mu \, \mathcal{N} \, {}^2}{q \, \mathrm{det} \, \umatrix} \braket{\trial \psi_{\umatrix} | H - E | \trial \psi_{\umatrix}}.
\end{equation}
$${#eq-kvp-general}
Note that Equation @eq-kvp-general is not restricted to coordinate space, _e.g._, it also holds for scattering in momentum space (see Reference @KVP_vs_NVP:2022).
...
Variational principles may not always provide a (unique) stationary approximation, causing the appearance of spurious singularities known as Kohn (or Schwartz) anomalies [@Drischler:2021qoy;@Lucchese:1989zz;@nesbet1980variational], which can render applications of VPs ineffective; especially for sampling of a model's parameter space.^[These anomalies are not restricted to the KVP but also appear in other VPs such as the NVP and SVP @ADHIKARI1991435]
The appearance of these anomalies depends on the parameters $\params$ used to train the emulator in the offline stage, the scattering energy, and the evaluation set used in the online stage.
...
Alternatively, one might also consider comparing the consistency of emulated results obtained from different VPs such as the ones summarized in Table @tbl-scattering-vps-galerkin.
...
$$
\begin{align}
    \dU_{ij}^{ss'} & \equiv \ \frac{2 \mu \, \mathcal{N} \, {}^2}{q \, \mathrm{det} \, \umatrix}
    \big[
    \braket{\psi_i^{s} | H(\params) - E | \psi_j^{s'}} %\notag \\
    %& \qquad \qquad \,
    + (i \leftrightarrow j) \notag
    %& \qquad \quad \, + \braket{\psi_j^{st} | [H(\params) - E]^{tt'} | \psi_i^{t's'}} \notag
    \big]
    \notag\\
    %& \quad\null +  \braket{\psi_j^{st} | [H - E]^{tt'} | \psi_i^{t's'}} \notag\\
    & =  \frac{2 \mu \, \mathcal{N} \, {}^2}{q \, \mathrm{det} \, \umatrix}
    \big[
    \braket{\psi_i^{s} | V(\params) - V_j | \psi_j^{s'}} %\notag \\
    %& \qquad \qquad \,
    + (i \leftrightarrow j) \notag
    %& \qquad \quad \, + \braket{\psi_j^{st} | [V(\params) - V_i]^{tt'} | \psi_i^{t's'}} \notag
    \big], \notag\\
    %& \quad\null +  \braket{\psi_j^{st} | [V(\params) - V_i]^{tt'} | \psi_i^{t's'}},
    % \label{eq:delta_u_tilde_coupled}
\end{align}
$${#eq-delta_u_tilde_coupled}
with $s$ and $s'$ corresponding to the entrance and exit channels.
...
The coefficients are independent because $\mathcal{L}^{ss'}$ is independently stationary for each $ss'$ pair.
This becomes apparent when considering how one would solve for the coefficients in the case where there are two uncoupled channels, where $ss' \to \ell$ in Equations @eq-kvp_reduced_coupled and @eq-delta_u_tilde_coupled.
Here, each partial wave is completely independent of one another and thus each VP and their corresponding coefficients $\coeffs$ are independent of one another across values of $\ell$.
...
Here, we follow Reference [@Zhang:2021jmi], which developed KVP-based emulators for three-body systems.
...
The trial wave function has the following asymptotic behavior~\cite{Zhang:2021jmi}:
$$
...
The variational functional with these inputs produce accurate results for the $S$ matrix at the emulation points @Zhang:2021jmi.
...
From the potential @eq-separable-potential, simple expressions for $K$ and $\psi$ can be derived @PhysRevC.55.1650.
...
\end{align}
$$ {#eq-yamaguchi}
with $\ell=0$ and assume a rank-2 potential with $b_i = [2, 4]$ and $2\mu = 1$.
In this case,
$$
...
We can see that all but the constrained KVP are extremely accurate, with the residuals mostly governed by the choice of nugget used to regularize the matrix inversion (see also Section @sec-eigen-emulators_variational).
For the constrained KVP, we see that the loss of a degree of freedom to implement the constraint significantly impacts its predictive power given that we only have two basis states, although it is still quite accurate in this case.
Increasing the basis to $\nbasis = 5$ yields predictions that are accurate to $10^{-13}$ degrees, or better, for all emulators.
...
#| fig-cap: "Wave functions (top panel) and absolute residuals (bottom panel) for the Yamaguchi potential @eq-yamaguchi in arbitrary units using the constrained KVP. The top panel legend description is similar to Figure~\ref{fig:yamaguchi_phase_shifts}, but for three different values of $q$. The bottom panel shows the relative residuals of the three values previously mentioned."
...
The emulator reproduces the high-fidelity solution at all three values of $q$, with $q = 2.0$ having the smallest residual.
The sensitivity of the emulator accuracy as $\nbasis$ is varied can be readily studied using the Python code provided on the companion website @companionwebsite. An example of how the accuracy is affected when varying $\nbasis$ is also given in Refereence @KVP_vs_NVP:2022.
...
The constrained KVP has the advantage that terms constant in $\params$, such as the (long-range) Coulomb potential, cancels in the computation of Equation @eq-delta_u_tilde but it loses one degree of freedom due to the normalization constraint of the coefficients.
