...
Similar to the discussion in Section~\ref{sec:eigen-emulators_variational}, the matrix $A_s$ in Equation @eq-coefficient_solve_quadratic may be ill-conditioned and require regularization.
A nugget $\nu \ll 1$ can be added to the diagonal elements of $A_s$ to help stabilize the solution for $\coeffsopt$ [@Neumaier98solvingill-conditioned;@engl1996regularization].

## Galerkin Emulators

On the other hand, the Galerkin approach, also more broadly called the "method of weighted residuals," relies on the _weak} formulation of the differential equations @eq-generic_differential_and_boundary.
