...
Given a (Hermitian) Hamiltonian $H(\params)$ parameterized by $\params$, we aim to find the solutions $\{E(\params), \ket{\psi(\params) }\}$ of the Schrödinger Equation
$$
...
subject to the normalization $\braket{\psi(\params)|\psi(\params)} = 1$.
...
We consider here cases in which Equation @eq-generic_eigenvalue_problem can be solved with high fidelity, but doing so requires a significant amount of compute time. This compute time is compounded when repeated solutions are required throughout the parameter space, \eg, during optimization routines or Monte Carlo sampling.
In the following, we will discuss how the Ritz variational principle and the Galerkin method can be used to construct rapid and reliable^[A _reliable_ emulator may not necessarily be required to be highly accurate, _e.g._, if the other uncertainties of the theoretical calculation dominate the overall uncertainty budget.] emulators that facilitate these calculations.
...
For a critical commentary on the history of the method's name, see, _e.g._, References [@LEISSA2005961;@ILANKO2009731].%
...
%\end{subequations}
$$ {#eq-trial_general}
where the column-vector $\coeffs$ contains the to-be-determined coefficients and the row-vector^[In a representation of $H$, the $\psi_i$ corresponding to $\ket{\psi_i}$ are the $\nbasis$ columns of the matrix $X$ in that representation.] $X$ the (in principle) arbitrary basis states.
...
Inspired by Figure 2.1 in Reference @Benner2017modelRedApprox.](figs/rbm.png){#fig-illustration_rbm}


Figure @fig-illustration_rbm motivates the efficacy of snapshot-based trial functions.
...
From the stationarity condition @eq-stationarity_condition, we obtain the reduced-order model @Melendez:2020xcs
$$
...
As opposed to $H(\params)$ in Equation @eq-generic_eigenvalue_problem, $\subspace{H}(\params)$ (and likewise $\subspace{N}$) is a $\nbasis \times \nbasis$ Hermitian matrix
$$
\begin{equation}
    %\renewcommand*{\arraystretch}{1.1}
  \subspace{H}(\params) =
...
In practice, the generalized eigenvalue problem @eq-ritz_condition} may experience numerical instabilities due to small singular values in $\subspace{N}$.
(The instabilities also appear in reduced-order modeling of differential equations, see Section @sec-model-reduction.)
One way to ameliorate these instabilities is to orthonormalize the snapshots in $X$, hence yielding $\subspace{N} = \identity$.
This approach could also permit some efficiency gains, by excluding the least important vectors as measured by their singular values.
Alternatively, Reference @Hicks:2022ovs recently introduced a trimmed sampling algorithm that can substantially reduce the effects of noise in solving generalized eigenvalue problems.
Finally, a well-known approach to regularize both generalized eigenvalue problems and matrix inversion is through the use of a nugget [@Neumaier98solvingill-conditioned;@engl1996regularization].
Here, a regularization parameter $\nu\ll 1$ (called a nugget) is added to the diagonal of the ill-conditioned matrix one wishes to invert (here, $\subspace{N}$), thus shifting its singular values.

By solving the generalized eigenvalue problem @eq-ritz_condition,
...
If the snapshot basis $X$ in the trial wave function @eq-trial_general contains $\nbasis$ linearly independent states, then the Min--Max Theorem @Suzuki:1998bn asserts that each Lagrange multiplier,
$$
...
with the subspace-projected $\widetilde{O}(\params)  = \ritzbasis^\dagger O(\params) \ritzbasis$. However, these expectation values generally do not provide variational bounds unless $O = H$ is the Hamiltonian, as discussed (see, _e.g._, Figure 5 in Reference @Konig:2019adq for emulated \isotope[4]{He} ground-state radii).
...
To this end, we construct the _weak form_ of the Schrödinger Equation @eq-generic_eigenvalue_problem by left-multiplying it by an arbitrary test function $\bra{\testfunc}$ and asserting that
$$
...
With the degrees of freedom for $\ket{\psi}$ reduced, we enforce a less strict orthogonality condition: we select $\nbasis$ test functions $\testfunc_i$ and assert that the residual due to the trial wave function (cf. @fig-illustration_rbm) should be orthogonal to the subspace $\mathcal{Z}$ spanned by these test functions $Z = [\ket{\testfunc_1}, \dots, \ket{\testfunc_{\nbasis}}]$:
$$
\begin{align}
...
But replacing $\ket{\psi} \to \ket{\trial\psi}$ also implies that the true eigenvalue $E$ is in general not exactly reproduced unless $\mathcal{X}$ contains $\ket{\psi(\params)}$.
...
For brevity, the figure assumes that the snapshots are orthonormalized during the offline stage such that $\subspace{N} = \identity$ in the emulator equation @eq-ritz_condition.
...
The predictions are made quickly and with little memory footprint in the online stage.
...
These affine operators can be expressed as a sum of products of parameter-dependent functions $h_n(\params)$ and parameter-independent operators $H_n$,
$$
...
This (optional) preprocessing step can be performed during the offline stage, as illustrated in Figure @fig-illustration_fom_rom, thereby rendering the emulator equation @eq-ritz_condition an eigenvalue problem (_i.e._, $\subspace{N} = \identity$) and less sensitive to numerical noise.
...
The potential @eq-anharm_osc has the affine structure defined in Equation @eq-H_affine for $\params$ and hence can be emulated rapidly after projecting into the snapshot basis during the offline stage.
...
#| fig-cap: "Absolute residuals of the emulated wave functions (in fm$^{-1/2}$) based on the RBM and HO emulator as a function of $r$, with colors corresponding to those in Figure @fig-eigen-emulator-wavefunctions. The emulator results are compared to the exact solutions. See the main text for details."
#| echo: false
...
The sensitivity of the emulator accuracy as $\nbasis$ is varied can be readily studied using the Python code provided on the companion website @companionwebsite.
...
We choose to show the results of emulating the radius-squared operator $R^2$, defined here to be
$$
\begin{align}
    \braket{\psi | R^2 | \psi} = \frac{1}{\mathcal{N}} \int_0^\infty r^2\dd{r} r^2\psi^2(r) ,
\end{align}
$$
with the normalization $\mathcal{N} = \int_0^\infty r^2\dd{r} \psi^2(r)$.
...
$$
\begin{align}
    \subspace{R}_{ij}^2 \equiv \braket{\psi_i | R^2 | \psi_j} = \frac{1}{\mathcal{N}} \int_0^\infty r^2 \dd{r} r^2\psi_i(r)\psi_j(r)
\end{align}
$$
and then the online stage emulation can occur quickly via Equation @eq-ec_expectation_emulator; _i.e._, $\braket{\psi(\params) | R^2 | \psi(\params)} \approx \sum_{ij} \coeffsopt^{(i)}(\params)\subspace{R}_{ij}^2 \coeffsopt^{(j)}(\params)$.
...
#| fig-cap: "Absolute residuals in the energy (in MeV) at the 50 validation points for the RBM, HO, and GP emulators. The validation points are chosen randomly from a uniform distribution within the same range as the training points. See the main text for details."
#| echo: false
...
#| fig-cap: "Similar to Figure @fig-energy-residuals but for the root-mean-squared radius $\\sqrt{\\langle{R^2}\\rangle}$ and in units of fm.."
#| echo: false
...
Furthermore, its ability to extrapolate beyond the support of its training data is often poor unless great care is taken in the design of its kernel and mean function (see Figures 1 and 2 in Reference @Konig:2019adq).
The GP suffers from what, in other contexts, could be considered its strength: because it treats the high-fidelity system as a black box, it cannot use the structure of the high-fidelity system to its advantage (although some information can be conveyed via physics-informed priors for the hyperparameters).
...
With these strengths, the RBM emulator is highly effective in emulating bound-state systems, even with only a few snapshots and far from the support of the snapshots (see Figures1 and 2 in Reference @Konig:2019adq).
