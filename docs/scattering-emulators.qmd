...
$$
    K = \left(\mathbb{1} - V G_0 \right)^{-1} V,
$$ {#eq-LS_formal_sol}
...
$$
\begin{aligned}
    m_i(\param) & = \bra{\phi'} \left[K_i G_0 V(\param) + V(\param) G_0 K_i\right] \ket{\phi}, \label{eq:m_vec} \\
    M_{ij}(\param) & = \bra{\phi'} [
                       K_i G_0 K_j - K_i G_0 V(\param) G_0 K_j \notag \\
    & \hspace{0.29in} + K_j G_0 K_i - K_j G_0 V(\param) G_0 K_i
    ] \ket{\phi}. \label{eq:M_mat}
\end{aligned}
$$
...
This results in substantial improvements in CPU time, \eg, for chiral nucleon-nucleon (NN) interactions.

By imposing the stationary condition $\dd \mathcal{K} /\mathup{d} \coeffs = 0$, one then finds
...
$$ {#eq-emulator}
Equations~\eqref{eq:m_vec}--\eqref{eq:emulator} are the main expressions for emulating scattering observables
